...
---
[YesWorkflow](https://github.com/yesworkflow-org/yw-prototypes)  is a tool that models conventional scripts and exposes the underlying workflow view (**prospective** provenance). Firstly, a YesWorkflow user can add special YesWorkflow (YW) comments to existing scripts. These comments declare how data is used and results are produced, step by step, by the script. Then, the YesWorkflow tool interpret the YW comments and produce a graphical output that reveals the stages of computation and the flow of data in the script. The YesWorkflow annotation table is available at [here](). Lastly, YesWorkflow provides query capability for the prospective provenance of the scripts. We use [Alice's soil mapping script]({{site.baseurl}}/data/C3_C4_map_present_NA.m) to demonstrate the usage of the YesWorkflow tool.
